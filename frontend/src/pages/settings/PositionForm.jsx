...
const PositionForm = () => {
  ...
  const fetchPosition = async () => {
    try {
      setLoading(true);
      const response = await settingsAPI.getPositionById(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await settingsAPI.updatePosition(id, formData);
        toast.success('Position updated successfully!');
      } else {
        await settingsAPI.createPosition(formData);
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {isEditing ? 'Update position information' : 'Add a new position'}
            </p>
          </div>
        </div>
      </div>
